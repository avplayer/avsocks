...
private:
	void typedetect(const boost::system::error_code & ec);
	void handle_client_ssl_handshake(const boost::system::error_code & ec);
	void handle_avserver_connected(const boost::system::error_code & ec){
		if(!ec){
			//执行 ssl handshake
  			m_sslstream->async_handshake(ssl::stream_base::client,boost::bind(&avclient::handle_client_ssl_handshake,shared_from_this(),asio::placeholders::error));
		}else{
	...
	m_socket_server(_io_service)
{
	...
	//m_sslstream_server(m_socket_server,m_sslctx);
}
...
void avclient::typedetect(const boost::system::error_code& ec)
{
	...
	//TODO 检查 HTTP 协议.
		...
		m_sslstream->async_handshake(ssl::stream_base::server,
			boost::bind(&avclient::handle_client_ssl_handshake,shared_from_this(),asio::placeholders::error)
		);
	}
}

void avclient::handle_client_ssl_handshake(const boost::system::error_code& ec)
{
	if(!ec){
